...
public class FtpServerThread extends Thread {
	...
	static int FTP_SERVER_PORT = 9096;
	...
	public FtpServerThread() {
		...
		this.serverAddress=serverSocket.getLocalAddress();
		//packet data length + FTP header (12 bytes) + UDP header (8 bytes)
		buffer = new byte[PACKET_DATA_LENGTH+20];
	...
	public void run() {
		...
		while (moreFile) {
			...
			//"connect" - accepts only connection from 1st client, who sends SYN packet.
			if ((packet.isSYN())&&(clientAddress==null)){
				//populate addresses
				...
				//if this is a first packet in a series after the established "connection"
    ...
    //writes data into the file. FILE MUST BE OPENED FIRST!!!
    public void writeData(byte[] data) {
    	try {
			fileWriter.write(data);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			System.out.print("Error writing to the file");
			e.printStackTrace();
		}
    }
    
    //flushes buffer and closes file - to do at the end only
	...
	//receives incoming data, outputs FTPPacket object
	public FTPPacket receiveData() {
		...
		System.out.println(packet.Length + " " + packet.ACK + " " + packet.SYN + " " + packet.FIN);
		return packet;
	}
